...
def zoom_image(data, num):
    """
    data numpy
    reshape image to 96*96 pixels  
    """
    if num==3:
        x, y, c = data.shape
        zoomed_image = zoom(data, (256 / x, 256 / y, 1))
    elif num==2:
        x, y = data.shape
        zoomed_image = zoom(data, (256 / x, 256 / y))

    return zoomed_image

# def cvsize_image(data):

#     cvsized_image = cv2.resize(data, (256, 256))

#     return cvsized_image
...
def comp_class_vec(ouput_vec, index=None):
    ...
    index = index[np.newaxis, np.newaxis] #1x1
    index = torch.from_numpy(index)
    one_hot = torch.zeros(1, 2).scatter_(1, index, 1)
    one_hot.requires_grad = True
    class_vec = torch.sum(one_hot * ouput_vec.squeeze(2).squeeze(2)) 
