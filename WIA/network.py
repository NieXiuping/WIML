...
class scSE(nn.Module):
    def __init__(self, in_channels):
        ...
        self.cSE_block = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels//2, in_channels, kernel_size=1), #512/2，在python3中的结果是256.0,但是在python2中就是256 把/换成//，后者的运算能够保证是int型数据
            nn.Sigmoid()
        )
        #sSE
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        ...
        return seg_result, cls_result, feature1, feature2, feature3
...
class FPSNet(nn.Module):
    ...
    def forward(self, x):
        seg_result, cls_result, feature1, feature2, feature3 = self.net(x)
